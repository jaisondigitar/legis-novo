    ...
    public function show($id)
    {
        ...
        if (count($lawsProjectReference) > 0) {
            foreach ($lawsProjectReference as $reference) {
                ...
                if ($reference->justify) {
                    $pdf->AddPage();
                    $pdf->setListIndentWidth(5);
                    $content = '<h3 style="text-align: center">JUSTIFICATIVA</h3>';

                    $content .= '<p>'.$reference->justify.'</p>';
...
//            if ($request->file('law_file')->move('laws', $fileName)) {}
    ...
    public function replyLawsProjects($id, Request $request)
    {
        $lawProject = $this->lawsProjectRepository->findById($id);

        $lawProject->description = $request->description;
        $lawProject->file_reply = $request->file;
        $lawProject->save();
    }
}
